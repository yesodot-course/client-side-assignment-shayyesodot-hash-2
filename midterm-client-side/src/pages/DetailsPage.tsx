...
import { Box, Button, Card, CardContent, CardMedia, Chip, Container, Divider, Grid, TextField, Typography } from '@mui/material';
...
const DetailsPage = () => {
  ...
  if (isLoading) return <Typography align="center" sx={{ mt: 10 }}>טוען...</Typography>;
  if (!item) return <Typography align="center" sx={{ mt: 10 }}>המוצר לא נמצא</Typography>;
    ...
    <Container maxWidth="lg" sx={{ py: 6 }} dir="rtl">
      ...
      <Card sx={{ borderRadius: 4, background: 'linear-gradient(145deg, #1e2130, #262a3d)', overflow: 'hidden' }}>
        <Grid container sx={{ flexDirection: 'row-reverse' }}>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <CardContent sx={{ p: 6, display: 'flex', flexDirection: 'column', height: '100%', textAlign: 'right' }}>
              <Box sx={{ mb: 2 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 4, flexDirection: 'row-reverse', justifyContent: 'flex-start' }}>
              ...
              <Box sx={{ mt: 'auto', display: 'flex', alignItems: 'flex-end', gap: 3, flexDirection: 'row-reverse' }}>
                <Box>
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>כמות</Typography>
                  <TextField
                    type="number"
                    value={quantity}
